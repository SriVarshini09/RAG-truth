...
**Model:** `cross-encoder/nli-deberta-v3-large` (HuggingFace)  
**Task:** For each claim, determine whether the reference passage **entails**, **contradicts**, or is **neutral** toward it.

We use a cross-encoder NLI model that takes ⟨reference, claim⟩ as input and outputs a three-class probability distribution. Claims are processed in batches of 16 for GPU efficiency. The model runs locally with no API cost.

**Output per claim:**
```json
{"label": "CONTRADICTION", "confidence": 0.91, 
 "scores": {"ENTAILMENT": 0.02, "NEUTRAL": 0.07, "CONTRADICTION": 0.91}}
```
...
2. If **≥50% of claims are NEUTRAL** → hallucination = True (baseless info)
...
| **Multi-Agent (ours)** | None | TBD | TBD | **TBD** | TBD | TBD | TBD |

> *Multi-agent results to be inserted after pipeline completion.*
...
|--------|-------------|-----|
| A: Claim Only | Any claims extracted → hallucinated (no NLI) | TBD |
| B: NLI on Full Response | No decomposition, NLI on whole response | TBD |
| C: A1+A2, Majority Vote | Claims + NLI, simple >50% threshold | TBD |
| D: Full Pipeline (ours) | A1+A2+A3 with custom rules | TBD |
...
| Multi-Agent (ours) | 600 (GPT) | Optional | ~$0.15 |
...
We presented a zero-shot multi-agent pipeline for hallucination detection in RAG systems. By decomposing responses into atomic claims and verifying each against source passages using NLI, our system achieves [RESULT] overall F1 on the RAGTruth benchmark without any task-specific training. Ablation studies confirm that claim decomposition and NLI verification each contribute meaningfully, while our custom aggregation rules outperform simple majority voting. 
